...
public class Zapato extends Applet{
    ...
    private BranchGroup escenario(){
        ...
        TextureLoader archivo = new TextureLoader("src\\Fondojava\\fondozapatos.jpg",this); //se carga la imagen del fondo
        ...
        objetoRaiz.addChild(zapato()); //se llama al metodo modelar objeto
        objetoRaiz.addChild(iluminar());
        return objetoRaiz;
    }
    private BranchGroup zapato(){
        BranchGroup objetoRaiz = new BranchGroup();
        TransformGroup grupo = new TransformGroup();
        Transform3D transformacion3d = new Transform3D();
        transformacion3d.setTranslation(new Vector3d(0.049,0.24,-5.27)); //define posicion del objeto
        transformacion3d.setScale(0.020); //define el tamaño del objeto
        ...
        producto.addChild(cargarObjeto("src\\Objeto\\deportivo.obj")); //se llama el metodo cargarObjeto y se le envia como parametro la ruta del archivo
        
        Alpha alpha = new Alpha(-1, Alpha.INCREASING_ENABLE | Alpha.DECREASING_ENABLE, 0,0,10000,0,0,10000,0,0);
        
        Transform3D eje = new Transform3D();
        eje.setRotation(new AxisAngle4f(0.3f,4.0f,-1.0f,5.0f));
        RotationInterpolator position = new RotationInterpolator(alpha,producto,eje,90.0f,(float) Math.PI*(8.0f));
